...
const updateDepartment = async (req, res) => {
    ...
    try {
      ...
      if (!department) {
        return res.status(404).json({ message: `No department matches ID ${req.body.id}.` });
      }
  
      if (req.body.did === department.did && req.body.name === department.name) {
        return res.status(400).json({ message: 'No changes made to the department.' });
      }
      ...
      if (existingDepartment) {
        return res.status(409).json({ message: 'Department ID or Name is already used.' });
      }
